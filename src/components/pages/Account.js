...
const Account = ({
  reservation: { dates, apartment, trips, car },
  cancelReservation,
  setAlert,
}) => {
  const reservationCancellation = () => {
    if (
      window.confirm('Are you sure you want to cancel the whole reservation?')
    ) {
      cancelReservation();
  ...
  if (dates !== null && apartment !== null) {
    return (
      <Wrapper>
          ...
          <ApartmentCard>
            <Link to={`/apartment/${apartment.id}`}>
              <ApartmentImg>
                ...
                <div>
                  <span>
                    {apartment.bathrooms} <i className="fa fa-bath"></i>
                  </span>
                  <span>
                    {apartment.bedrooms} <i className="fa fa-bed"></i>
                  </span>
                </div>
              </ApartmentImg>
              <ApartmentName>
        ...
        <Key>
          <p>
            You can grab the apartment <i className="fa fa-key"></i> at{' '}
            <span>9:00 am</span> on reservation day
          </p>
          <p>
            Also, you must give the key back at the latest by{' '}
            <strong>9:00 pm</strong> the reservation ending day
          </p>
        </Key>
          ...
          <Fragment>
            ...
            <TripsPopulated>
              {trips.map((adventure, index) => {
                return (
                  <TripsText key={index}>
                    Adventure with <img src={adventure.guide.img} alt="" />{' '}
                    {adventure.guide.name} on{' '}
                    <strong style={{ margin: '0 5px' }}>
                      {adventure.trip.name}
                    </strong>{' '}
                    starting on {adventure.trip.date} at{' '}
                    {adventure.trip.departure}{' '}
                  </TripsText>
                );
              })}
            </TripsPopulated>
        ...
        {car === null ? (
          ...
          <Fragment>
            ...
            <CarWrapper>
              <div>
                Superfast {car.type} {car.mark} with {car.engine} engine is
        ...
        <ButtonCancel onClick={reservationCancellation}>
...
export default connect(mapStateToProps, { cancelReservation, setAlert })(
  Account
);
