...
const TripTrip = styled.div`
  ...
  ul {
    ...
    @media (min-width: 581px) {
      grid-template-columns: 88px 80px 68px 150px 60px 60px;
    }
    @media (max-width: 580px) {
      grid-template-columns: auto auto auto minmax(auto, 150px);
      justify-content: inherit;
      border-top: 1px solid black;
      &.trip > li:not(:first-child) {
...
const GuideDetails = ({ props, setTrip, setAlert, reservation }) => {
  ...
  useEffect(() => {
    ...
    if (!guide) {
      ...
      guidesData.find(
        (guide) => guide.name.toLowerCase() === name && setThisGuide(guide)
      );
    }
  ...
  const TripReservation = (trip) => {
    ...
    var theSame = reservation.trips
      ? reservation.trips.filter(
          (same) =>
            same.trip.id === trip.id && same.guide.name === thisGuide.name
        )
      : null;
      ...
      <Wrapper>
        ...
        <Grid>
          <GridLeft>
            ...
            <a href={`tel:+${thisGuide.phone}`}>{thisGuide.phone}</a>
          </GridLeft>
          ...
          <GridRight>
            <GridRightTextContainer>
              ...
              Adipisci vero quisquam blanditiis voluptas ut deserunt impedit
              libero ipsa perspiciatis, minus quis dicta, harum reprehenderit
              illum. Lorem ipsum dolor sit amet consectetur adipisicing elit.
              Perspiciatis, deleniti quas quia beatae totam ipsum?
            </GridRightTextContainer>
          </GridRight>
        </Grid>
        ...
        <ExpertiseGrid>
          ...
          <ExpertisePerks>
            {thisGuide.expertise.map((perk, id) => (
              <li key={id}>{perk}</li>
            ))}
          </ExpertisePerks>
        </ExpertiseGrid>
                  ...
                  <li>
                    {reservation.apartment !== null ? (
                      ...
                      <button
                        onClick={() =>
                          setAlert(
                            'Firstly, you need to make an apartment reservation',
                            'red',
                            3000
                          )
                        }
