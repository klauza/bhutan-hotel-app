...
const Picker = ({ setCalendarDates, setApartment, setAlert, apartment }) => {
  ...
  const reservation = () => {
    ...
    setTimeout(() => {
      setAlert('You can now hire a guide or rent a car', 'green', 3250);
    }, 2000);
  };
  ...
  return (
    <div>
      ...
      {showDate ? (
        <ReservationDates>
          <p>
            <strong>From:</strong> {date[0].toLocaleDateString('en-GB')}
          </p>
          <p>
            <strong>To:</strong> {date[1].toLocaleDateString('en-GB')}
          </p>
        </ReservationDates>
      ) : null}
      {showDate ? (
        <p>
          <strong>
            ...
            <i
              ...
              style={{ marginLeft: '5px', fontSize: '1.15rem' }}
            ></i>
          </strong>
        </p>
      ) : null}
      ...
      {showReservation ? (
        <ReservationButton onClick={reservation}>
          Make reservation
        </ReservationButton>
      ) : null}
    </div>
...
export default connect(null, { setCalendarDates, setApartment, setAlert })(
  Picker
);
